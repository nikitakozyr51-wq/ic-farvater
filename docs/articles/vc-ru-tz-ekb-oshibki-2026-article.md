...
Партия пришла. Всё красиво упаковано, накладные в порядке, входной контроль пройден. А через три недели выясняется, что компоненты не подходят под диапазон рабочих температур объекта. И вроде бы всё по ТЗ. И поставщик ни при чём. И виноватых нет. Только сроки горят, а перевыпуск платы стоит денег и нервов.

Такие ситуации случаются регулярно. Не потому что поставщики плохие или снабженцы невнимательные. Просто ТЗ на поставку ЭКБ пишут... по-разному. Иногда это листок из Word на три строки, иногда — 40-страничный документ, половину которого составляют общие фразы без единого числа. Разберём конкретные ошибки, которые потом всплывают уже после получения партии.
...
Торговая марка в ТЗ — это пожелание, не техническое требование. Пишите параметры, а не бренд.
...
Бывает обратная ситуация: поставщик молча кладёт функциональный аналог, потому что оригинала нет и давно нет, и думает что "и так понятно". А у заказчика КД с конкретным обозначением. Приёмка не проходит. Начинается переписка, корректировка документов, время.
...
Без этой строчки у поставщика нет оснований ни для какого решения. И у него, и у вас потом будут вопросы.
...
Для изделий, которые проходят испытания по ГОСТ (в том числе в аккредитованных лабораториях типа ЭКБ ТЕСТ), диапазон определяется программой испытаний. Этот документ должен быть исходником при написании ТЗ на поставку, а не формироваться после.
...
## Отдельный жанр ужасов: ТЗ, написанное маркетологом
...
Если у вас есть стандартный шаблон ТЗ и вы не меняли его последние несколько лет — стоит пройтись по нему с этим списком. Что из перечисленного там есть, а чего нет?

Проверить наличие конкретных позиций и уточнить требования к документации на поставку можно на [ic-farvater.ru](https://ic-farvater.ru) или по телефону +7 (996) 778-88-42.
